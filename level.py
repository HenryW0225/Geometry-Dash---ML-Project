class Spike :
    ...
    def visible_coordinates(self, screen_world_x, screen_width) :
...
class Level :
    ...
    def add_spike(self, world_x, height = 50, width = 50) :
        newSpike = Spike(world_x, self.floor, height, width) 
        self.spikes.append(newSpike)
    
    def floor(self) :
        return self.floor
    
    def update_spikes(self, screenWorldX, SCREEN_WIDTH, block_x, block_y, block_size) :
        spikeCoordinates = []
        for spike in self.spikes :
            coordinates = spike.visible_coordinates(screenWorldX, SCREEN_WIDTH) 
